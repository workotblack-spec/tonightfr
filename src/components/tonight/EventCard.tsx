...
import { CATEGORIES, type Event } from "@/data/events";
    ...
    >
      <div className="relative aspect-[16/10] overflow-hidden">
        ...
        <button
          ...
          className="absolute right-3 top-3 grid h-10 w-10 place-items-center rounded-full glass transition-transform active:scale-90"
        >
          <Heart
      ...
      <div className="space-y-2 p-4">
        ...
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          ...
          {event.price && (
            <span className="shrink-0 font-medium text-foreground/80">{event.price}</span>
          )}
        </div>
      </div>
    </article>
